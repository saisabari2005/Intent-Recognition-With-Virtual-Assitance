...
with open("ai-virtual-assistance/intends.json") as file:
    data = json.load(file)
...
def browsing(query):
    if 'google' in query:
        ...
        webbrowser.open(f"{s}")
    # elif 'brave' in query:
    #      speak("opening your brave ")
    #      s = command().lower()
    #      hi.startfile("C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe")
...
def condition():
    ...
    if percentage>=80:
        speak(" we could have enough charging to continue our work")
    elif percentage>=40 and percentage<=75:
        speak(" we should connect our system to charging point to charge our battery")
    else:
        speak(" we have very low power, please connect to charging otherwise  should be off...")

    
if __name__ == "__main__":
    wishMe()
    # engine_talk("Allow me to introduce myself, the virtual artificial intelligence and I'm here to assist you with a variety of tasks as best I can, 24 hours a day seven days a week.")
    while True:
        query = command().lower()
